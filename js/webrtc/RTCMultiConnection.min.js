...
var RTCMultiConnection = function(roomid, forceOptions) {
    ...
    function onMessageCallback(data) {
        ...
        "rtcmulticonnection-extension-loaded" == data && (chromeMediaSource = "desktop"), data.sourceId && screenCallback && screenCallback(sourceId = data.sourceId, data.canRequestAudioTrack === !0)
    }
    ...
    function getSourceId(callback) {
        ...
        return sourceId ? callback(sourceId) : (screenCallback = callback, void window.postMessage("get-sourceId", "*"))
    }

    function getSourceIdWithAudio(callback) {
        if (!callback) throw '"callback" parameter is mandatory.';
        return sourceId ? callback(sourceId) : (screenCallback = callback, void window.postMessage("audio-plus-tab", "*"))
    }
    ...
    function getScreenConstraints(callback, captureSourceIdWithAudio) {
        ...
        return "desktop" != chromeMediaSource || sourceId ? ("desktop" == chromeMediaSource && (screen_constraints.mandatory.chromeMediaSourceId = sourceId), void callback(null, screen_constraints)) : void(captureSourceIdWithAudio ? getSourceIdWithAudio(function(sourceId, canRequestAudioTrack) {
    ...
    ! function(connection) {
        ...
        mPeer.onGettingLocalMedia = function(stream, callback) {
            ...
            options = options || {};
            var cb = function() {};
            "function" == typeof options && (cb = options, options = {}), "undefined" != typeof options.localPeerSdpConstraints && (localPeerSdpConstraints = options.localPeerSdpConstraints),
                "undefined" != typeof options.remotePeerSdpConstraints && (remotePeerSdpConstraints = options.remotePeerSdpConstraints), "undefined" != typeof options.isOneWay && (isOneWay = options.isOneWay), "undefined" != typeof options.isDataOnly && (isDataOnly = options.isDataOnly);
            var connectionDescription = {
                remoteUserId: connection.sessionid,
                message: {
                    newParticipationRequest: !0,
            ...
            return session.getTracks ? (connection.attachStreams.indexOf(session) === -1 && (session.streamid || (session.streamid = session.id), connection.attachStreams.push(session)), void connection.renegotiate(remoteUserId)) : isData(session) ? void connection.renegotiate(remoteUserId) : void((session.audio || session.video || session.screen) && (session.screen ? "Edge" === DetectRTC.browser.name ? navigator.getDisplayMedia({
                ...
                return error ? "PermissionDeniedError" === error ? (session.streamCallback && session.streamCallback(null), void(connection.enableLogs && console.error("User rejected to share his screen."))) : alert(error) : void connection.invokeGetUserMedia({
                    audio: !!isAudioPlusTab(connection) && getAudioScreenConstraints(screen_constraints),
                    video: screen_constraints,
                    isScreen: !0
                }, function(stream) {
        ...
        }, connection.trickleIce = !0, connection.version = "3.6.8", connection.onSettingLocalDescription = function(event) {
